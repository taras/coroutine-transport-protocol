 ...
 *
 * Kill this process mid-cook with `pkill -f "demo/cook.ts"`, then restart.
...
const SERVER_URL = Deno.env.get("DURABLE_SERVER_URL") ?? "http://localhost:4437";
const STREAM_ID = Deno.env.get("DURABLE_STREAM_ID") ?? "dinner-demo";

// Fresh producerId each run — avoids seq/epoch bookkeeping in the demo
const PRODUCER_ID = `cook-${crypto.randomUUID().slice(0, 8)}`;
