...
#   deno task demo [stream-id]    # same thing
#
...
tmux send-keys -t "${SESSION}:0.0" "deno task demo:server" Enter

# Give the server a moment to bind its port
sleep 2

# Pane 1: Start the journal tailer
tmux send-keys -t "${SESSION}:0.1" "deno task demo:tail" Enter

# Pane 2: Pre-type the cook command (presenter hits Enter when ready)
tmux send-keys -t "${SESSION}:0.2" "deno task demo:cook"
